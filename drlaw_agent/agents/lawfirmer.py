...
from drlaw_agent.agents.toolkits.lawfirm import lawfirm_info_tools


class LawfirmerAgent:
    """
    律师事务所信息助理:
    能根据律师事务所名称提供该律师事务所相关信息和统计数据
    """
    ...
    async def run(self, drlaw_state: dict):
        print_agent_output("律师事务所信息助理正在查询信息", agent="LAWFIRMER")

        task = drlaw_state.get("task")
        recursion_limit = task.get("recursion_limit")
        query = drlaw_state.get("sub_query")[-1]

        system_message = """你是一个擅长回答与律师事务所信息有关的问题的专家，会通过调用工具来检索信息。
...
"""
        ...
        app = create_react_agent(model, lawfirm_info_tools, system_message, debug=True)
